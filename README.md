...
1. A tree is constructed in parallel on the GPU. The tree data structure is provided by our SpatialCL library (see http://github.com/illuhad/SpatialCL). This tree is a perfectly balanced binary tree constructed over the stars sorted along a Hilbert curve (note that this allows Teralens to efficiently process clustered star distributions as well!). While this tree choice can lead to an overlapping of the nodes (which may require more nodes to be accessed), it allows for a very fast tree construction.
...
We would like to emphasize that, unlike earlier GPU-accelerated quasar microlensing codes (e.g. Thompson 2010), Teralens works _entirely_ on the GPU. The only data transfers taking place between CPU and GPU are the star sample in the beginning (copied to the GPU), and, in the end, the final magnification pattern (copied back to the CPU). In between these two steps, the calculation is fully carried out on the GPU. For future versions, is is planned to move the sampling of the stars to the GPU as well.
...
## Credits
In general, you are free to use Teralens under the conditions of the GNU General Public License 3.0, but if you intend to use Teralens for scientific publications, it is kindly requested:
...
* You do _not_ need hardware from one specific vendor. Teralens should work with any GPU vendor (NVIDIA, AMD, in principle even Intel integrated GPUs). Unfortunately, at the moment we only have NVIDIA GPUs available for testing, but we strive for wide compatibility. If you have a GPU from a different vendor and encounter problems, it is considered a bug and we will do our best to fix it.
...
* Teralens also runs decently on CPUs, if you do not have a GPU available at all. However, you will still need an OpenCL implementation from your CPU vendor (e.g. Intel). Additionally, most of our optimization efforts target GPUs. It will run faster on a GPU with the same power consumption.
...
* `cmake`
* `cfitsio`
* An OpenCL implementation supporting at least OpenCL 1.2. Teralens is tested with the NVIDIA and Intel OpenCL implementations, but should also work on AMD hardware (but we did not have AMD GPUs available for testing at the moment -- reports of the experience of using Teralens on AMD GPUs or CPUs are highly appreciated!)
...
* Teralens also depends on our SpatialCL and QuickCL libraries. See below how to fulfill these dependencies.
